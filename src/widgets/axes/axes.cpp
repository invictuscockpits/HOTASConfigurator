    ...
    , ui(new Ui::Axes)
{
    ...
    // set a2b  // Is this duplicating work from readFromConfig()
...
void Axes::on_pushButton_StartCalib_clicked(bool checked)
{
    ...
    if (checked == true) {
        ...
        ui->spinBox_CalibMax->setValue(AXIS_MIN_VALUE); // Keep in order
...
void Axes::setComboBox_AxisSource1Visibility(bool enabled) {
    ui->comboBox_AxisSource1->setVisible(enabled);
}
void Axes::setText_axisSourceVisibility(bool enabled) {
    ui->text_axisSource->setVisible(enabled);
}
\
    void Axes::setCalibrationMin(int minValue)
{
    qDebug() << "Setting Calibration Min to:" << minValue;
    ui->spinBox_CalibMin->setValue(minValue);
}

void Axes::setCalibrationMax(int maxValue)
{
    qDebug() << "Setting Calibration Max to:" << maxValue;
    ui->spinBox_CalibMax->setValue(maxValue);
}
