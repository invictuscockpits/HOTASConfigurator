...
void AxesToButtonsSlider::paintEvent(QPaintEvent *event) // optimize width_ - offset_*2
{
    Q_UNUSED(event)
    QPainter painter;
    int rect_height = 5;
    ...
    if (m_pointPtrList.size() >= 2) {
        int x1 = m_pointPtrList[0]->posX;
        int x2 = m_pointPtrList[1]->posX;
        if (x2 < x1) std::swap(x1, x2); // Ensure left-to-right

        QRect activeZoneRect(x1, rect_y, x2 - x1, rect_height);
        painter.setBrush(m_axisRectColor);
        painter.setPen(Qt::NoPen);
        painter.drawRect(activeZoneRect);
    }
    // paint separators
    ...
    // Antialiasing     // Sloppy code, may not work
    ...
    if (pointPos.x() < m_kOffset || pointPos.x() > this->width() - m_kOffset) {
        ...
        if (m_pointsCount > 1) {
            if (pointNumber > 0 && pointNumber < m_pointsCount - 1) {
                if (uint(pointPos.x()) < m_pointPtrList[pointNumber - 1]->posX + m_kRangeBetween // Bug when loading config?
                    || uint(pointPos.x()) > m_pointPtrList[pointNumber + 1]->posX - m_kRangeBetween) // if the points are too close, rounding occurs
                {
                    return;
                }
...
void AxesToButtonsSlider::pointsPositionReset()
{
    ...
    for (int i = 0; i < m_pointPtrList.size(); ++i) {
        if (this->isEnabled() == true) {
            ...
            m_pointPtrList[i]->color = Qt::darkGray;
