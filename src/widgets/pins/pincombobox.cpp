...
int PinComboBox::getIndexForGuiName(const QString &guiName) const
{
    for (int i = 0; i < m_pinTypesIndex.size(); ++i) {
        if (m_pinTypes[m_pinTypesIndex[i]].guiName == guiName) {
            return i;
        }
    }
    return -1;
...
uint PinComboBox::getPinNumber() const {
    return m_pinNumber;
}
