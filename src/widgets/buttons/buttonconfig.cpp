...
#else
    for (int i = 0; i < MAX_LOGICAL_BUTTONS_GUI; i++) {
...
void ButtonConfig::createLogButtons(int count)
{
    ...
    for (int i = 0; i < count; i++) {
        if (size == MAX_LOGICAL_BUTTONS_GUI) {
            break;
        }
...
void ButtonConfig::logScrollValueChanged(int value)
{
    ...
    int size = m_logicButtonPtrList.size();
    if (size != MAX_LOGICAL_BUTTONS_GUI && (value / float(scroll->maximum())) >= 1.0f)
    {
        createLogButtons(4);
    }
}
...
void ButtonConfig::logicaButtonsCreator()
{
    ...
    if (tmp >= MAX_LOGICAL_BUTTONS_GUI) {
       if (MAX_LOGICAL_BUTTONS_GUI != 32) { //changing to 32 max buttons
            qCritical() << "buttonconfig.cpp MAX_BUTTONS_NUM != 32"; //changing to 32 max buttons
        }
        qDebug() << "LogicaButtonsCreator() finished in"<<timer.elapsed()<<"ms";
        emit logicalButtonsCreated();
        return;
    }
    // As I understand it, the timer triggers after the application has fully loaded (it will be displayed)
    // Because LogicaButtonsCreator is entered during initialization, but it triggers after the application starts running.
    QTimer::singleShot(10, this, [&] {
        ...
        #endif
#else
        // MAX_BUTTONS_NUM(128)/8 = 16 Must be divisible without a remainder
        for (int i = 0; i < MAX_LOGICAL_BUTTONS_GUI; i++) // old value =8 // useless atm
        {
...
//            }
    ...
    // shift state
    for (int i = 0; i < SHIFT_COUNT; ++i) // Looks like redundant code, but it's necessary for optimization
    {
        if (paramsRep->shift_button_data & (1 << (i & 0x07))) {
            ...
            if (i == 0 && m_shift1_act == false) {
                ...
                ui->text_shift1_logicalButton->setStyleSheet(m_defaultShiftStyle + "background-color: rgb(5, 170, 61);");
                //ui->groupBox_Shift1->setStyleSheet("background-color: rgb(5, 170, 61);");
                //ui->spinBox_Shift1->setStyleSheet("background-color: rgb(5, 170, 61);");
                m_shift1_act = true;
            } else if (i == 1 && m_shift2_act == false) {
                m_defaultShiftStyle = ui->text_shift1_logicalButton->styleSheet();
                ui->text_shift2_logicalButton->setStyleSheet(m_defaultShiftStyle + "background-color: rgb(5, 170, 61);");
                m_shift2_act = true;
            } else if (i == 2 && m_shift3_act == false) {
                m_defaultShiftStyle = ui->text_shift1_logicalButton->styleSheet();
                ui->text_shift3_logicalButton->setStyleSheet(m_defaultShiftStyle + "background-color: rgb(5, 170, 61);");
                m_shift3_act = true;
            } else if (i == 3 && m_shift4_act == false) {
                m_defaultShiftStyle = ui->text_shift1_logicalButton->styleSheet();
                ui->text_shift4_logicalButton->setStyleSheet(m_defaultShiftStyle + "background-color: rgb(5, 170, 61);");
                m_shift4_act = true;
            } else if (i == 4 && m_shift5_act == false) {
                m_defaultShiftStyle = ui->text_shift1_logicalButton->styleSheet();
                ui->text_shift5_logicalButton->setStyleSheet(m_defaultShiftStyle + "background-color: rgb(5, 170, 61);");
                m_shift5_act = true;
            }
...
const QList<ButtonLogical*>& ButtonConfig::logicButtons() const
{
    return m_logicButtonPtrList;
}
...
void ButtonConfig::readFromConfig()
{
    ...
    // dynamic creation with scroll
#ifdef DYNAMIC_CREATION
    for (int i = MAX_LOGICAL_BUTTONS_GUI; i > 0; --i) {
