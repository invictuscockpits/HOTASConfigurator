...
AxesCurvesConfig::AxesCurvesConfig(AxesConfig* axesConfig, QWidget *parent)
 :
    QWidget(parent),
    ui(new Ui::AxesCurvesConfig)
{
    ...
    if (auto axesConfig = parent->findChild<AxesConfig*>()) {
        connect(axesConfig, &AxesConfig::axisVisibilityChanged,
                this, &AxesCurvesConfig::handleAxisVisibility);
    }
    ...
    if (axesConfig) {
        QTimer::singleShot(0, this, [axesConfig, this]() {
            for (int i = 0; i < MAX_AXIS_NUM; ++i) {
                QCheckBox* chb = axesConfig->findChild<QCheckBox *>(axesList()[i].guiName);
                if (chb) {
                    bool isVisible = !chb->isChecked();
                    handleAxisVisibility(i, isVisible);
                    ...
                    if (i >= 0 && i < labelNames.size()) {
                        if (auto label = findChild<QLabel*>(labelNames[i]))
                            label->setVisible(isVisible);
                    }

                } else {
                    qDebug() << "Could not find checkbox for axis:" << axesList()[i].guiName;
                }
            }
        });
    }
...
void AxesCurvesConfig::handleAxisVisibility(int index, bool visible)
{
    if (index >= 0 && index < m_profiles.size())
        m_profiles[index]->setVisible(visible);
    ...
    if (index >= 0 && index < labelNames.size()) {
        if (auto label = findChild<QLabel*>(labelNames[index])) {
