...
void ShiftRegistersConfig::addPinAndSort(int pin, const QString &pinGuiName, std::array<ShiftRegData_t, MAX_SHIFT_REG_NUM + 1> &arr)
{
    ...
    // sort ascending
    std::sort(arr.begin(), arr.end(), sortByPinNumber);           // Not sure if this can be done with just one sort.
    // sort null last
...
void ShiftRegistersConfig::shiftRegSelected(int latchPin, int clkPin, int dataPin, const QString &pinGuiName)
{
    ...
    // add shift reg data pin
        ...
        std::sort(m_dataPinsArray.begin(), m_dataPinsArray.end(), sortByPinNumber);           // Not sure if it can be done with just one sort.
...
QList<ShiftRegisters*> ShiftRegistersConfig::getShiftRegisterWidgets() const
{
    return m_shiftRegsPtrList;
}
